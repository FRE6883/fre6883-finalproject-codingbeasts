...
namespace fre
{
    void plotResults(Vector Beat, Vector Meet, Vector Miss, int N) {
	    ...
	    FILE *gnuplotPipe,*tempDataFile, *temp;
	    int nIntervals = 2*N;
	...
	void write_to_stock(stringstream &sData, Stock_indv* stock)
	{
	...
	void FetchData(map<string, Stock_indv> &stock_map, String group_tickers)
	{
		...
		if (handle)
		{
			...
			string start_date = "2022-12-15";
			string end_date = "2023-12-15";
			...
			{	
				...
				Stock_indv *ticker =&stock_map[symbol];// &(itr->second);
