...
namespace fre
{
    std::vector<double> cumulativex(std::vector<double> input){
...
std::vector<double> calculateColumnAverages(const std::vector<std::vector<double>>& matrix) {
    ...
    if (!matrix.empty() && !matrix[0].empty()) {
        ...
        for (size_t col = 0; col < numCols; ++col) {
            double average = columnSums[col] / numRows;
            columnAverages.push_back(average);
        }
    }
    ...
    Bootstrap::Bootstrap(Group* GroupPtr_, map<string, Stock_indv>* MapPtr_, int n): GroupPtr(GroupPtr_),MapPtr(MapPtr_), T(2*n)
    {
    }

    // Random sample generator
    String Bootstrap :: GenS(int gr_n)
    {
        int max_size = (GroupPtr->Get_Group_Mapping_())[gr_n].size();
        String grouptickers(max_size);
        grouptickers = (GroupPtr->Get_Group_Mapping_())[gr_n];
        String W(M);
        vector<int> numbers;
        vector<int> sample;
    ...
    Vector Bootstrap :: Cumulative_Sum_(const Vector& V)
    {
        int d = (int)V.size();
        Vector U(d);
        U[0] = V[0];
    ...
    Vector Bootstrap :: Cal_AAR(int gr_no) 
    {
    ...
    void Bootstrap :: RunBS()  
    {
        int N_Group = GroupPtr->GetN(); 
        //initialize result matrices to 0s
        Avg_AAR = ConstMatrix(0,N_Group,T); 
        Avg_CAAR = ConstMatrix(0,N_Group,T);
        AAR_STD = ConstMatrix(0,N_Group,T); 
        CAAR_STD = ConstMatrix(0,N_Group,T);
        ...
        for(int n = 0; n < N_Group; n++) //iterate through each group
        {
            ...
            for(int i = 0;i < MCN; i++) //iterate through each monte carlo iteration (i.e. Bootstrap iteration 1-40) 
            {
                ...
                CAAR_tmp = Cumulative_Sum_(AAR_tmp);
    ...
    void Bootstrap::ResultsDisp(int n)
    {
        string group_name;
        if(n == 0)
            group_name = "Beat";
    ...
    void Bootstrap :: plott(Vector r1, Vector r2, Vector r3, int N) 
    {
